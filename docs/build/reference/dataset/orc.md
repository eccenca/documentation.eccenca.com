...
This dataset could be stored in a file `transactions.orc` and used in workflows where only certain columns or filtered rows are needed. Spark can read only the relevant columns or rows matching a condition, thanks to ORC’s **column pruning** and **predicate pushdown** optimizations, making transformations efficient even at large scale.
