...
In practical terms, Spark is used only in BUILD — not in EXPLORE or other components. Its purpose is to accelerate workflow execution for the so-called **Spark-aware datasets**, i.e., datasets executed via Spark, which include file formats and storage systems such as Avro, Parquet, ORC, Hive, and HDFS. These formats map naturally to Spark’s distributed processing model and benefit from in-memory execution and partition-based parallelism.
