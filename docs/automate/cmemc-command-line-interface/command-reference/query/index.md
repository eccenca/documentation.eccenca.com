...
```text
Usage: cmemc query open [OPTIONS] QUERIES...
...
```text
Usage: cmemc query status [OPTIONS] [QUERY_UUID]
...
It can be created with the `query status` command, e.g.
...
```text
Usage: cmemc query replay [OPTIONS] REPLAY_FILE
...
```shell-session
$ cmemc query cancel [OPTIONS] QUERY_ID
```

```text
Usage: cmemc query cancel [OPTIONS] QUERY_ID
