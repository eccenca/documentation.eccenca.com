...
class _IndentedDumper(yaml.Dumper):
    """Dumper that indents block sequences beneath their parent mapping key.

    PyYAML emits `key:` followed by items at the *same* column. yamllint rejects
    that under its default `indentation` rule, which sets `indent-sequences:
    true`. Forcing `indentless` off yields `key:` with the items indented under
    it, which is what the rule expects.
    """

    def increase_indent(self, flow: bool = False, indentless: bool = False):  # noqa: ARG002, FBT001, FBT002
        return super().increase_indent(flow, False)


# Prepended to the generated file. The `---` satisfies yamllint's
# `document-start` rule. The disable directive covers the handful of entries
# that pair a long title with a long docs-relative path: YAML folds plain
# scalars only at internal spaces, never after the `: `, so those lines cannot
# be wrapped and the rule can only ever be noise here.
NAV_HEADER = """\
---
# Generated by tools/build_navigation.py from the docs/**/.pages files.
# Do not edit by hand - run `task update:navigation` instead.
...
def render_nav(docs_dir: Path) -> str:
    ...
    # `allow_unicode` keeps characters as characters rather than \uXXXX escapes,
    # which matters for titles carrying entities such as &nbsp;.
    #
    # The indent stays at 2 because yamllint's `indentation` rule defaults to
    # `spaces: consistent`, which requires every indentation step in the file to
    # be the same width. A `- ` item prefix already costs two columns, so the
    # mapping key inside a sequence item sits two in from the dash; the sequence
    # step therefore has to be two as well. Four is unreachable: PyYAML's
    # `indent=4` pads the dash to `-   key:` and trips the `hyphens` rule, while
    # re-indenting by hand to `    - key:` trips `indentation` instead.
    body = yaml.dump(
        {"nav": nav},
